 ...
 * Bar chart for numeric metrics (years, scores, etc.)
 */
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        />
        <YAxis 
          tickFormatter={(value) => unit ? `${value}${unit}` : formatNumber(value, 1)} 
          stroke="#64748b"
          style={{ fontSize: '12px' }}
        />
        <Tooltip 
          formatter={(value) => unit ? `${(value as number).toFixed(1)}${unit}` : formatNumber(value as number, 1)}
